...
public class ValidateBinarySearchTree_98_2 {

    public ValidateBinarySearchTree_98_2(Integer[] nums) {
        this.nums = nums;
    }
    public ValidateBinarySearchTree_98_2() {
    }
    ...
    public static void main(String[] args) {
        Solution solution = new ValidateBinarySearchTree_98_2().new Solution();

        Integer[] nums =
                {5,1,4,null,null,3,6};
        ValidateBinarySearchTree_98_2 obj = new ValidateBinarySearchTree_98_2(nums);
        obj.buildtree();
...
//leetcode submit region begin(Prohibit modification and deletion)
    ...
    class Solution {
        ...
        private boolean validate(TreeNode root) {
            ...
            // 找出左樹中最大的值
            TreeNode leftNode = getMax(root.left);
            if(leftNode!=null && currentVal<=leftNode.val)
                result = false;
            // 找出右樹中最小的值
            TreeNode rightNode = getMin(root.right);
            if(rightNode!=null && currentVal>=rightNode.val){
                result = false;
            }



            // DFS (Depth-first search) left
            boolean r1 = validate(root.left);
            boolean r2 = validate(root.right);
